...
static void inj_color_picker_cb(App_Color_ID color_id) {}

static void on_headlight_btn_press(lv_event_t *lv_event) {
...
void View_Accessory_Create(View_Base_t *self, lv_obj_t *parent) {
  ...
  lv_obj_add_event_cb(headlight_btn, on_headlight_btn_press, LV_EVENT_CLICKED,
                      self);

  lv_color_t hl_btn_color = UI_Helper_Get_LV_Color(APP_COLOR_YELLOW);
  ...
  lv_obj_set_grid_cell(bodylight_btn, LV_GRID_ALIGN_CENTER, 1, 1,
                       LV_GRID_ALIGN_CENTER, 0, 1);
  lv_obj_add_event_cb(bodylight_btn, on_bodylight_btn_press, LV_EVENT_CLICKED,
                      self);

  lv_color_t bl_btn_color = UI_Helper_Get_LV_Color(APP_COLOR_YELLOW);
